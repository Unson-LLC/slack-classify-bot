 ...
 * 日付をISO 8601形式に変換
 */
function toISOString(date: string, endOfDay: boolean = false): string {
  // 既にISO形式の場合はそのまま返す
  if (date.includes('T')) {
    return date;
  }
...
export const calendarFindCommonAvailabilityTool = createTool({
  ...
  execute: async (input) => {
    ...
    try {
      ...
      // 空き時間を計算
      const availableSlots: { start: string; end: string; durationMinutes: number }[] = [];
      ...
      while (currentDate < searchEnd) {
        ...
        // この日のbusy時間を取得
        const dayBusy = allBusySlots.filter(slot =>
...
export const calendarGetTodayScheduleTool = createTool({
  ...
  execute: async (input) => {
    ...
    try {
      ...
      // 全カレンダーのイベントを取得
      const allEvents: (ReturnType<typeof formatEvent> & { calendarId: string })[] = [];
